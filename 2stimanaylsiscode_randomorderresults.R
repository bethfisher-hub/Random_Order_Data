...
dftrials <- dftrials[which(dftrials$ID != 5),]
...
# Remove participant 5 as they did not get high enough catch score
dftrials <- dftrials[which(dftrials$ID!=5),]
...
dissimplot_temporal <- function(subjectdf,colors,dependent='color'){
    ...
    plot <- ggplot(datatemp, aes(x = Group.1, y = Group.2)) +
      theme(axis.text.x = (element_text(colour = colors, angle = 90, size=14)), axis.text.y = element_text(colour = colors, size=14),
            plot.title = element_text(hjust = 0.5, size = 30), text = element_text(size=24), plot.subtitle =element_text(size=16))
    
    
    # stuff that's standard across plot types
    plot <- plot + geom_raster(aes(fill = similarity)) +
      labs(title = 'Dissimilarity Plot', x = 'Presented Screen', y = 'Response Screen', subtitle = 'N=14 
(324/participant)') +
...
df <- data.frame(matrix(unlist(corlist), nrow=length(corlist), byrow=T))

corrandom = c(0.9590745, 0.9247234, 0.9105580, 0.8983738, 0.8457288, 0.7531941, 0.9543627, 0.9156286, 0.9065987,
 0.8559000, 0.7019380, 0.8942657, 0.9587003, 0.8478747)

cor1st2nd = c(0.9454244, 0.9686079, 0.9243651, 0.9011141, 0.9744665, 0.9883750, 0.9070221, 0.9178963, 0.9600219,
              0.9343422, 0.9610065, 0.9692182, 0.9212921, 0.9236159)
corflower = c(0.9357045, 0.9167604, 0.8248977, 0.6965186, 0.7739744, 0.8133771, 0.8266048, 0.8004041, 0.7637221,
               0.9561925,0.7901334, 0.9237635)
cor250ms = c(0.936549, 0.9591413, 0.9631909, 0.8263699, 0.980105, 0.9256297, 0.9654683, 0.9022425, 0.9668899,
              0.9613479, 0.9721026, 0.8617546) 

Data <- data.frame(
  Y=c(corrandom, cor1st2nd, corflower, cor250ms),
  Experiment =factor(rep(c("corrandom", "cor1st2nd", "corflower", "cor250ms"), times=c(length(corrandom), length(cor1st2nd), length(corflower), length(cor250ms))))
)

Data <- data.frame(
  correlation=c(corrandom, cor1st2nd,cor250ms),
  Experiment =factor(rep(c("corrandom", "cor1st2nd", "cor250ms"), times=c(length(corrandom), length(cor1st2nd), length(cor250ms))))
)

TukeyHSD(fm1)


fm1 <- aov(correlation~Experiment, data=Data)
anova(fm1)
...
matdfall <- rbind(matdf1, matdf2, matdf3, matdf4, matdf6, matdf7, matdf8, matdf9, matdf10, matdf11, matdf12, matdf13, matdf14, matdf15)
...
asymmetry_plot_temporal <- function(subjectdf, colors){
  ...
  plot <- plot + geom_raster(aes(fill = asymmetry)) +
    labs(title = 'Asymmetry Plot with p-values', x = 'Presented Screen', y = 'Response Screen', subtitle = 'N=14 
(324/participant)') +
